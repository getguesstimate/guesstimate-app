...
(function(React, ReactRouter, Reflux, FermActions, fermListStore, fermEditingStore, global) {
    ...
    var Node = React.createClass({
      ...
      handlePress: function(e) {
        if (e.keyCode == 68) { this.handleDestroy() }
      },
      handleChoose: function() {
        this.props.updateEditingNode(this.props.node.id);
      },
      render: function() {
        var node = this.props.node;
        return (
          <div className={"panel panel-default bottom-panel-" + node.type} onClick={this.handleChoose}>
              <div className="panel-heading" activeClassName="panel-warning">
...
//-          positions =  d.position
    ...
    var FermApp = React.createClass({
      ...
      getEditingNode: function(){
        return fermListStore.getItem(this.state.editingNode);
      },
      getDecorateList: function(){
        EditingNode = _.find(this.list, function(item){
          return item.id === itemId;
        })
    ...
    var routes = (
      <Route handler={FermApp}>
        <Route name="All" path="/" handler={FermApp} />
      </Route>
    );

    ReactRouter.run(routes, Router.HashLocation, function(Handler) {
      React.render(<Handler/>, document.getElementById('fermapp'));
    });
})(window.React, window.ReactRouter, window.Reflux, window.FermActions, window.fermListStore, window.fermEditingStore, window);
