...
import {getNodeAncestors, containsDuplicates, getMissingInputs, isDescendedFromFn, withInputIndicesFn} from 'lib/DAG/nodeFns'
...
function expandCyclesAndAddGraphErrors(component, nodeAncestors) {
  ...
  return withMissingInputErrors.map(heightSet => {
    return heightSet.map(withAncestralErrorFn(_.flatten(withMissingInputErrors), nodeAncestors))
  })
}
...
export class SimulationDAG {
  constructor(nodes) {
    ...
    if (containsDuplicates(nodes)) { console.warn('DUPLICATE IDs DETECTED'); return }
    ...
    const {acyclicNodes, cycleSets} = getCycleSets(withInputs, this.nodeAncestors)
    const heightSets =  separateIntoHeightSets([...acyclicNodes, ...cycleSets.map(toCyclePseudoNode)])
    const processedHeightSets = _.flatten(expandCyclesAndAddGraphErrors(heightSets, this.nodeAncestors))
  ...
  subsetFrom(idSet) { return this.nodes.filter(orFns(n => idSet.includes(n.id), isDescendedFromFn(idSet, this.nodeAncestors))) }
  strictSubsetFrom(idSet) { return this.nodes.filter(isDescendedFromFn(idSet, this.nodeAncestors)) }
}
