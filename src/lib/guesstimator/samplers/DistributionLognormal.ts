import { log as mathLog, mean as mathMean } from "mathjs";
import { Sampler, simulate } from "./Simulator";

export const sampler: Sampler = {
  async sample({ params: [low, high] }, n, _1) {
    // This assumes a centered 90% confidence interval, e.g. the left endpoint
    // marks 0.05% on the CDF, the right 0.95%.
    const logHigh = mathLog(high) as number;
    const logLow = mathLog(low) as number;

    const mean = mathMean(logHigh, logLow);
    const stdev = (logHigh - logLow) / (2 * 1.645);

    return simulate(`lognormal(${mean},${stdev})`, [], n);
  },
};
