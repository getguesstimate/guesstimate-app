...
const BAD_ENDING_WORDS = ['of', 'in', 'on', 'per', 'for', 'and', 'but', 'or', 'each']
...
function processWord(rawWord) {
  ...
  if (word.length >= 9) {
    ...
    if (strippedWord.length > 3) { word = strippedWord }
...
function getDirectVariableNameFromName(rawName, maxOneWordLength, maxSplitWordLength, cutOffLength, makeAcronym) {
  const name = prepareName(rawName)

  const words = name.split(/[\_]/).map(processWord).filter(s => !_.isEmpty(s))
  ...
  if (words.length === 1 && name.length < maxOneWordLength) {
    ...
    if (makeAcronym) {
      return _.trimEnd(words.map(readableIdPartFromWord).slice(0, cutOffLength).join(''), '_')
    } else {
...
export function getVariableNameFromName(
  ...
  let directName = getDirectVariableNameFromName(rawName, maxOneWordLength, maxSplitWordLength, totalMaxLength, makeAcronym)
  ...
  const suffixes = matchingNames.map(v => parseInt(v.match(nameRegex)[1] || '0'))
  if (!suffixes.includes(0)) { return directName }
