...
describe('nodeFns', () => {
  describe('getNodeAncestors', () => {
    ...
    it ('Correctly assigns nodes their relations', () => {
      ...
      expect(nodeAncestors['J']).to.have.members(['G', 'I', 'H', 'A', 'F', 'D', 'C', 'B'])
  ...
  describe('anyRelationsWithinFn', () => {
    ...
    describe('correctly catches relationships', () => {
      ...
      it('correctly excludes non-relatives', () => { expect(testFn({id: '2', inputs: ['0']})).to.be.false })
      ...
      it('correctly flags descendants', () => { expect(testFn({id: '4', inputs: ['3', '2']})).to.be.true })
      it('correctly flags ancestors', () => { expect(testFn({id: 'A', inputs: []})).to.be.true })
    })
  })
  ...
  describe('containsDuplicates', () => {
    it ('correctly returns false with an empty list', () => {expect(containsDuplicates([])).to.be.false})
    it ('correctly returns false with a list with no dupes', () => {
      expect(containsDuplicates([{id: '0', inputs: []}, {id: '1', inputs: ['0']}, {id: '2', inputs: ['0']}])).to.be.false
    })
    it ('correctly returns true with a list with dupes', () => {
      expect(containsDuplicates([{id: '0', inputs: []}, {id: '1', inputs: ['0']}, {id: '0', inputs: ['1']}])).to.be.true
    })
  })

  describe('withInputIndicesFn', () => {
    it ('correctly extracts input indices', () => {
      const nodes = [{id: '0', inputs: []}, {id: '1', inputs: ['0']}, {id: '2', inputs: []}, {id: '3', inputs: ['0', '2', 'missing']}]
      expect(withInputIndicesFn(nodes)({id: '3', inputs: ['0', '2', 'missing']})).to.deep.equal({
        id: '3',
        inputs: ['0', '2', 'missing'],
        inputIndices: [0, 2],
      })
