import * as EventTypes from './constants.js'
...
function trackAddedProduct(planType) {
  window.analytics.track(EventTypes.ADDED_PRODUCT_ACTION, {
    id: planType,
    name: planType
  })
}
...
function trackViewedAllProducts() {
  window.analytics.track(EventTypes.VIEWED_PRODUCT_ACTION, {
    id: LITE_PLAN,
    name: LITE_PLAN
  })
  window.analytics.track(EventTypes.VIEWED_PRODUCT_ACTION, {
...
export function trackUsedViewMode(mode) {
  window.analytics.track(EventTypes.USED_VIEW_MODE, {mode})
}
...
export function trackPurchaseSuccess(account, rawPlanType) {
  ...
  window.analytics.track(EventTypes.PURCHASE_SUCCESS_ACTION, {
    orderId: orderId,
    products: [
      {
        id: planType,
...
export function trackAccountModalClick() {
  window.analytics.track(EventTypes.ACCOUNT_MODAL_CLICK)
}

export function trackUserMenuOpen() {
  window.analytics.track(EventTypes.USER_MENU_OPEN)
}

export function trackUserMenuClose() {
  window.analytics.track(EventTypes.USER_MENU_CLOSE)
}
