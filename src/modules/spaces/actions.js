...
export function fromSearch(data) {
  return (dispatch) => {
    const formatted = data.map(d => _.pick(d, ['id', 'name', 'description', 'user_id', 'updated_at', 'metric_count', 'is_private', 'screenshot', 'large_screenshot']))
  ...
  return (dispatch, getState) => {
    ...
    api(getState()).models.list({userId, organizationId}, (err, value) => {
      if (err) {
        ...
        const formatted = value.items.map(d => _.pick(d, ['id', 'name', 'description', 'user_id', 'organization_id', 'updated_at', 'metric_count', 'is_private', 'screenshot', 'large_screenshot']))
